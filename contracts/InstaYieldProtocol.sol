...
import "./interfaces/IInstaYieldStrategy.sol";
import "./interfaces/IInstaYieldToken.sol";
import "./interfaces/IInstaYieldNFT.sol";
import "./interfaces/IInstaYieldTokenFactory.sol";

contract InstaYieldProtocol is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    address public immutable InstaYieldNFTAddress;
    address immutable InstaYieldFTokenFactoryAddress;
    ...
    constructor(address _InstaYieldNFTAddress, address _InstaYieldFTokenFactoryAddress) public {
        InstaYieldNFTAddress = _InstaYieldNFTAddress;
        InstaYieldFTokenFactoryAddress = _InstaYieldFTokenFactoryAddress;
    }
    ...
    function registerStrategy(
        ...
        address InstaYieldFToken = IInstaYieldTokenFactory(InstaYieldFTokenFactoryAddress).createFToken(_fTokenName, _fTokenSymbol);

        // Store the appropriate information
        strategies[_strategyAddress].fTokenAddress = InstaYieldFToken;
        strategies[_strategyAddress].principalTokenAddress = _principalTokenAddress;

        IInstaYieldStrategy(_strategyAddress).setFTokenAddress(InstaYieldFToken);

        emit StrategyRegistered(_strategyAddress, _principalTokenAddress, InstaYieldFToken);
    }
    ...
    function stake(
        ...
        require(
            _stakeDuration >= 60 && _stakeDuration <= IInstaYieldStrategy(_strategyAddress).getMaxStakeDuration(),
            "ISD"
        );
        ...
        uint256 tokensToMint = IInstaYieldStrategy(_strategyAddress).quoteMintFToken(_tokenAmount, _stakeDuration);

        // Deposit into the strategy
        uint256 principalAfterDeductions = IInstaYieldStrategy(_strategyAddress).depositPrincipal(_tokenAmount);

        // Calculate fee and if this is more than 0, transfer fee
        uint256 fee = (tokensToMint * globalMintFee) / 10000;
        if (fee > 0) {
            IInstaYieldToken(strategies[_strategyAddress].fTokenAddress).mint(globalMintFeeRecipient, fee);
        }

        // Mint fERC20 tokens to the user
        IInstaYieldToken(strategies[_strategyAddress].fTokenAddress).mint(_fTokensTo, (tokensToMint - fee));
    ...
    function unstake(
        ...
        if (_isNFT) {
            ...
            require(IInstaYieldNFT(InstaYieldNFTAddress).ownerOf(_id) == msg.sender, "NNO");
        } else {
            stakeId = _id;
            p = stakes[stakeId];
        ...
        if (percentageIntoStake >= (10**18)) {
            ...
            // Burn these fTokens
            IInstaYieldToken(strategies[p.strategyAddress].fTokenAddress).burnFrom(msg.sender, _fTokenToBurn);
        ...
        IInstaYieldStrategy(p.strategyAddress).withdrawPrincipal(principalToReturn);
    ...
    function issueNFT(uint256 _stakeId) public returns (uint256 _nftId) {
        ...
        // Mint the NFT
        uint256 nftId = IInstaYieldNFT(InstaYieldNFTAddress).mint(msg.sender);
    ...
    function InstaYield(
        ...
        IInstaYieldStrategy(_strategyAddress).burnFToken(fTokensToUser, _minimumReceived, _yieldTo);
    }
}
