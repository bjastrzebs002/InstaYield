...
import { InstaYieldBackLM } from "../../typechain";
...
task("generate:InstaYieldBackLM")
  .addParam("InstaYieldBackaddress", "The token to be staked")
  .addParam("outputfile", "The tier, basically what to name the output file")
  .setAction(async function (taskArguments: TaskArguments, { ethers }) {
    const [wallet1] = await ethers.getSigners();

    const InstaYieldBackContract: InstaYieldBackLM = <InstaYieldBackLM>(
      await ethers.getContractAt("InstaYieldBackLM", taskArguments.InstaYieldBackaddress)
    );

    const _totalEntries = 100;
    const _minTokenAStaked = 0.001;
    const _maxTokenAStaked = 1000;
    const _minTimeStaked = (await InstaYieldBackContract.minimumStakeDuration()).toNumber();
    const _maxTimeStaked = (await InstaYieldBackContract.maximumStakeDuration()).toNumber();
    ...
    for (let i = 0; i < _totalEntries; i++) {
      ...
      let result = await InstaYieldBackContract.calculateReward(tokensAToStakeBN, timeToStake);
