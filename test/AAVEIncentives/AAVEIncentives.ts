...
import { InstaYieldStrategyAAVEv2 } from "../../typechain";
...
describe.skip("AAVEIncentive Test", function () {
  ...
  it("should claim DAI AAVE rewards and balance should be > 0", async function () {
    ...
    expect(await stakedAAVEToken.balanceOf(this.InstaYieldStrategyAAVEv2Artifact.address)).to.be.eq(0);
    ...
    await this.InstaYieldStrategyAAVEv2Artifact.claimAAVEv2Rewards(_assets, _amount);

    const currBalance = await stakedAAVEToken.balanceOf(this.InstaYieldStrategyAAVEv2Artifact.address);
