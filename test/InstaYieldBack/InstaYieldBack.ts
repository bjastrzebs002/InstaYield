...
import { InstaYieldBack, InstaYieldToken } from "../../typechain";
...
describe("InstaYieldBack Tests (same token)", function () {
  let InstaYieldTokenContract: InstaYieldToken;
  let InstaYieldBackContract: InstaYieldBack;
  ...
  before(async function () {
    ...
    const InstaYieldTokenContractA: Artifact = await hre.artifacts.readArtifact("InstaYieldToken");
    InstaYieldTokenContract = <InstaYieldToken>await deployContract(signers[0], InstaYieldTokenContractA);

    // Deploy the InstaYieldBack contract
    const InstaYieldBackContractA: Artifact = await hre.artifacts.readArtifact("InstaYieldBack");
    InstaYieldBackContract = <InstaYieldBack>(
      await deployContract(signers[0], InstaYieldBackContractA, [
        InstaYieldTokenContract.address,
        InstaYieldTokenContract.address,
        864000,
        31536000,
      ])
    ...
    await InstaYieldBackContract.connect(signers[0]).setMaxAPR(10000);
  });

  it("should transfer 5,000,000 Flash from account 0 to InstaYieldBack contract", async function () {
    let _amount = ethers.utils.parseUnits("5000000", 18);
    let _recipient = InstaYieldBackContract.address;

    await InstaYieldTokenContract.connect(signers[0]).transfer(_recipient, _amount);

    expect(await InstaYieldTokenContract.balanceOf(_recipient)).to.be.eq(_amount);
  ...
  it("account 1 should fail getting quote, 1,000,000 FLASH for 366 days with error DURATION > MAXIMUM IS 365 DAYS", async function () {
    ...
    await expect(InstaYieldBackContract.calculateReward(_amount, _duration)).to.be.revertedWith("DURATION > MAXIMUM");
  });
  ...
  it("account 1 should fail getting quote, 1,000,000 FLASH for < 10 days with error DURATION < MINIMUM", async function () {
    ...
    await expect(InstaYieldBackContract.calculateReward(_amount, _duration)).to.be.revertedWith("DURATION < MINIMUM");
  });
  ...
  it("account 1 should fail getting quote, 0 FLASH for < 10 days with error INSUFFICIENT INPUT", async function () {
    ...
    await expect(InstaYieldBackContract.calculateReward(_amount, _duration)).to.be.revertedWith("INSUFFICIENT INPUT");
  });
  ...
  it("(1) account 1 should stake 1,000,000 Flash for 36.5 days", async function () {
    ...
    // Approval and stake
    await InstaYieldTokenContract.connect(signers[1]).approve(InstaYieldBackContract.address, _amount);
    ...
    expect(await InstaYieldTokenContract.balanceOf(signers[1].address)).to.be.eq(0);
  });
  ...
  it("(2) account 2 should stake 536,432 Flash for 365 days", async function () {
    ...
    // Approval and stake
    await InstaYieldTokenContract.connect(signers[2]).approve(InstaYieldBackContract.address, _amount);
    ...
    expect(await InstaYieldTokenContract.balanceOf(signers[2].address)).to.be.eq(ethers.utils.parseUnits("463568", 18));
  });
  ...
  it("(3) account 2 should fail: stake 12,500 Flash for 365 days, _minimumReward = 12501 with error MINIMUM REWARD NOT MET", async function () {
    ...
    // Approval and stake
    await InstaYieldTokenContract.connect(signers[2]).approve(InstaYieldBackContract.address, _amount);
    await expect(InstaYieldBackContract.connect(signers[2]).stake(_amount, _duration, _minimumReward)).to.be.revertedWith(
  ...
  it("(3) account 2 should stake 12,500 Flash for 365 days", async function () {
    ...
    expect(await InstaYieldTokenContract.balanceOf(signers[2].address)).to.be.eq(ethers.utils.parseUnits("451068", 18));
  });

  it("account 1 should fail unstaking with error MINIMUM STAKE DURATION IS 10 DAYS", async function () {
    await expect(InstaYieldBackContract.connect(signers[1]).unstake(1)).to.be.revertedWith("DURATION < MINIMUM");
  });

  it("account 2 should fail unstaking account 1's stake with error NOT OWNER OF STAKE", async function () {
    await expect(InstaYieldBackContract.connect(signers[2]).unstake(1)).to.be.revertedWith("NOT OWNER OF STAKE");
  });
  ...
  it("(3a) account 1 should unstake early (stakeid 1), get back principal, rewards should redirect to treasury", async function () {
    ...
    const oldBalance = await InstaYieldTokenContract.balanceOf(signers[1].address);
    ...
    console.log("(3a) actualBalance", ethers.utils.formatUnits(await InstaYieldTokenContract.balanceOf(signers[1].address)));
    expect(await InstaYieldTokenContract.balanceOf(signers[1].address)).to.be.eq(expectedBalance);
    expect((await InstaYieldBackContract.stakes(1)).active).to.be.false;

    expect(await InstaYieldTokenContract.balanceOf(await InstaYieldBackContract.forfeitRewardAddress())).to.be.eq(
      stakeInfo.reservedReward,
    );
    expect(await InstaYieldTokenContract.balanceOf(forfeitAddress)).to.be.eq(stakeInfo.reservedReward);
  ...
  it("(4) account 2 should unstake, principal and reward to wallet", async function () {
    ...
    const oldBalance = await InstaYieldTokenContract.balanceOf(signers[2].address);
    ...
    expect(await InstaYieldTokenContract.balanceOf(signers[2].address)).to.be.eq(expectedBalance);
  ...
  it("(5) account 2 should unstake, principal and reward to wallet", async function () {
    const stakeInfo = await InstaYieldBackContract.stakes(3);
    const oldBalance = await InstaYieldTokenContract.balanceOf(signers[2].address);

    await InstaYieldBackContract.connect(signers[2]).unstake(3);

    const expectedBalance = oldBalance.add(stakeInfo.stakedAmount).add(stakeInfo.reservedReward);
    expect(await InstaYieldTokenContract.balanceOf(signers[2].address)).to.be.eq(expectedBalance);
    expect((await InstaYieldBackContract.stakes(3)).active).to.be.false;
  ...
  it("account 2 should attempt to unstake twice and fail with error INVALID STAKE", async function () {
    await expect(InstaYieldBackContract.connect(signers[2]).unstake(3)).to.be.revertedWith("INVALID STAKE");
  });

  it("account 0 should increase reward rate to: 63419583968 / 200%", async function () {
    await InstaYieldBackContract.connect(signers[0]).setMaxAPR("20000");
  });
  ...
  it("drain remaining rewards by staking 5 million Flash", async function () {
    ...
    // Approval and stake
    await InstaYieldTokenContract.connect(signers[0]).approve(InstaYieldBackContract.address, _amount);
    let result = await InstaYieldBackContract.connect(signers[0]).stake(_amount, _duration, _minimumReward);

    expect(await InstaYieldBackContract.getAvailableRewards()).to.be.eq(0);
  });

  it("ensure when there are no reward, staking results in error: INSUFFICIENT OUTPUT", async function () {
    await expect(InstaYieldBackContract.stake(1, 864000, 1)).to.be.revertedWith("INSUFFICIENT OUTPUT");
  });
});

describe("InstaYieldBack Tests (different reward token)", function () {
  let InstaYieldTokenContract: InstaYieldToken;
  let InstaYieldBackContract: InstaYieldBack;
  ...
  before(async function () {
    ...
    tokenB = <InstaYieldToken>await deployContract(signers[0], InstaYieldTokenContractA);
  ...
  it("(4) account 2 should unstake, principal and reward to wallet", async function () {
    const stakeInfo = await InstaYieldBackContract.stakes(2);
    const oldPrincipalBalance = await InstaYieldTokenContract.balanceOf(signers[2].address);
    ...
    expect(await InstaYieldTokenContract.balanceOf(signers[2].address)).to.be.eq(
    ...
    console.log("(4) newPrincipalBalance", await InstaYieldTokenContract.balanceOf(signers[2].address));
