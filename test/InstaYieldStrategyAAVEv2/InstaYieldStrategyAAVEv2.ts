...
import { InstaYieldFToken, InstaYieldStrategyAAVEv2 } from "../../typechain";
...
describe("InstaYieldStrategyAAVEv2 Tests", function () {
  ...
  before(async function () {
    ...
    const InstaYieldProtocolAddress = this.signers[1].address;
    const InstaYieldStrategyAAVEv2Artifact: Artifact = await hre.artifacts.readArtifact("InstaYieldStrategyAAVEv2");
    this.InstaYieldStrategyAAVEv2Artifact = <InstaYieldStrategyAAVEv2>(
      await deployContract(this.signers.admin, InstaYieldStrategyAAVEv2Artifact, [
        "0xE0fBa4Fc209b4948668006B2bE61711b7f465bAe",
        principalTokenAddress,
        yieldTokenAddress,
        InstaYieldProtocolAddress,
      ])
    );
    //console.log("-> AAVE DAI Strategy Deployed to", this.InstaYieldStrategyAAVEv2Artifact.address);
    strategyAddress = this.InstaYieldStrategyAAVEv2Artifact.address;

    // Create a new fERC20 token
    const InstaYieldFTokenArtifact: Artifact = await hre.artifacts.readArtifact("InstaYieldFToken");
    const InstaYieldFTokenContract = <InstaYieldFToken>(
      await deployContract(this.signers.admin, InstaYieldFTokenArtifact, ["fDAI", "fDAI"])
    );
    //console.log("Flash fERC20 token deployed to", InstaYieldFTokenContract.address);
    fTokenAddress = InstaYieldFTokenContract.address;

    // Set the FTokenAddress in the strategy
    await this.InstaYieldStrategyAAVEv2Artifact.connect(this.signers[1]).setFTokenAddress(InstaYieldFTokenContract.address);
    //console.log("Successfully set fToken address in strategy")

    // Attempt to set again and expect this to fail
    await expect(
      this.InstaYieldStrategyAAVEv2Artifact.connect(this.signers[1]).setFTokenAddress(InstaYieldFTokenContract.address),
    ).to.be.revertedWith("FTOKEN ADDRESS ALREADY SET");

    // Approve the Flash Strategy contract to spend fTokens from user wallet
    await InstaYieldFTokenContract.approve(
      this.InstaYieldStrategyAAVEv2Artifact.address,
  ...
  it("should ensure getFTokenAddress returns correct address", async function () {
    expect(await this.InstaYieldStrategyAAVEv2Artifact.getFTokenAddress()).eq(fTokenAddress);
  });

  it("should ensure getPrincipalAddress returns correct address", async function () {
    expect(await this.InstaYieldStrategyAAVEv2Artifact.getPrincipalAddress()).eq(principalTokenAddress);
  });

  it("should return error when getting quote: INSUFFICIENT fERC20 TOKEN SUPPLY", async function () {
    await expect(this.InstaYieldStrategyAAVEv2Artifact.quoteBurnFToken(1)).to.be.revertedWith(
  ...
  it("[account 0] ensure depositing principal fails with error NOT FLASH PROTOCOL", async function () {
    await expect(
      this.InstaYieldStrategyAAVEv2Artifact.depositPrincipal(BigNumber.from(10000).mul(multiplier)),
  ...
  it("[account 1] should deposit 10,000 DAI principal", async function () {
    ...
    await daiContract.connect(this.signers[0]).approve(this.InstaYieldStrategyAAVEv2Artifact.address, _amount);
    expect(await daiContract.allowance(this.signers.admin.address, this.InstaYieldStrategyAAVEv2Artifact.address)).eq(
      _amount,
    );

    await daiContract.connect(this.signers[0]).transfer(this.InstaYieldStrategyAAVEv2Artifact.address, _amount);
    expect(await daiContract.balanceOf(this.InstaYieldStrategyAAVEv2Artifact.address)).eq(_amount);
    ...
    expect(
      await this.InstaYieldStrategyAAVEv2Artifact.connect(this.signers[1]).callStatic.depositPrincipal(_amount),
    ).to.be.eq(_amount);

    // Deposit the x DAI in the Strategy Contract to AAVE
    await this.InstaYieldStrategyAAVEv2Artifact.connect(this.signers[1]).depositPrincipal(_amount);

    expect(await this.InstaYieldStrategyAAVEv2Artifact.getPrincipalBalance()).eq(_amount);
  });

  it("ensure yield increases after 100 blocks (AAVE)", async function () {
    const oldYieldTotal = await this.InstaYieldStrategyAAVEv2Artifact.getYieldBalance();

    await mineBlocks(hre.ethers.provider, 100);

    // Check how much yield has been generated
    const yieldGenerated = await this.InstaYieldStrategyAAVEv2Artifact.getYieldBalance();
  ...
  it("[account 0] should revert when withdrawing principal with error NOT FLASH PROTOCOL", async function () {
    await expect(
      this.InstaYieldStrategyAAVEv2Artifact.withdrawPrincipal(BigNumber.from(10000).mul(multiplier)),
    ).to.be.revertedWith("NOT FLASH PROTOCOL");
  });

  it("[account 1] should withdraw 10,000 DAI (principal) and still have yield in contract", async function () {
    // Withdraw the DAI
    await this.InstaYieldStrategyAAVEv2Artifact
      .connect(this.signers[1])
      .withdrawPrincipal(BigNumber.from(10000).mul(multiplier));

    // Ensure principal is 0
    const principalBalance = await this.InstaYieldStrategyAAVEv2Artifact.getPrincipalBalance();
    expect(principalBalance).eq(0);

    // Ensure yield is > 0
    const yieldBalance = await this.InstaYieldStrategyAAVEv2Artifact.getYieldBalance();
  ...
  it("should burn 5,000.00000256 fERC20 for totalYield / 2", async function () {
    ...
    const quotedAmount = this.InstaYieldStrategyAAVEv2Artifact.quoteBurnFToken(burnAmount);

    // Perform the burn
    const result = await this.InstaYieldStrategyAAVEv2Artifact.burnFToken(burnAmount, quotedAmount, this.signers[0].address);
  ...
  it("ensure quoting 1 DAI for 365 days = 1.000000000512000000 fDAI", async function () {
    ...
    expect(await this.InstaYieldStrategyAAVEv2Artifact.quoteMintFToken(_tokenAmount, _duration)).eq("1000000000512000000");
  });
  ...
  it("ensure quoting 1 DAI for 1 days = 0.0027397260288 fDAI", async function () {
    ...
    const result = await this.InstaYieldStrategyAAVEv2Artifact.quoteMintFToken(_tokenAmount, _duration);
  ...
  it("ensure quoting 0.000000000100000000 DAI for 60 seconds = 0.00000000000000190 fDAI", async function () {
    ...
    const _duration = 60;
    expect(await this.InstaYieldStrategyAAVEv2Artifact.quoteMintFToken(_tokenAmount, _duration)).eq("190");
  });
  ...
  it("ensure quoting 100,000 DAI for 1 days = 50000.000025600000000000 fDAI", async function () {
    ...
    expect(await this.InstaYieldStrategyAAVEv2Artifact.quoteMintFToken(_tokenAmount, _duration)).eq(
  ...
  it("ensure quoting for 59 seconds reverts with error DURATION TOO LOW", async function () {
    ...
    const _tokenAmount = BigNumber.from(1).mul(multiplier);
    const _duration = 59;
    await expect(this.InstaYieldStrategyAAVEv2Artifact.quoteMintFToken(_tokenAmount, _duration)).to.be.revertedWith(
      "DURATION TOO LOW",
    );
  });
  ...
  it("ensure quoting 1 DAI for 61 seconds = 0.000001934297312000 fERC20", async function () {
    ...
    const _tokenAmount = BigNumber.from(1).mul(multiplier);
    const _duration = 61;
    expect(await this.InstaYieldStrategyAAVEv2Artifact.quoteMintFToken(_tokenAmount, _duration)).eq("1934297312000");
  });
  ...
  it("ensure quoting 1 DAI for 182.5 days = 0.500000000256000000 fDAI", async function () {
    ...
    const _tokenAmount = BigNumber.from(1).mul(multiplier);
    const _duration = 31536000 / 2;
    expect(await this.InstaYieldStrategyAAVEv2Artifact.quoteMintFToken(_tokenAmount, _duration)).eq("500000000256000000");
  });
  ...
  it("ensure quoting 1 DAI for 3650 days = 10.00000000512000000 fDAI", async function () {
    ...
    expect(await this.InstaYieldStrategyAAVEv2Artifact.quoteMintFToken(_tokenAmount, _duration)).eq("10000000005120000000");
  });
  ...
  it("any ERC20 tokens in the contract can be withdrawn by owner except the interest bearing token (a token)", async function () {
    ...
    await erc20TokenContract
      .connect(signer)
      .transfer(this.InstaYieldStrategyAAVEv2Artifact.address, BigNumber.from(100).mul(multiplier));
    expect(await erc20TokenContract.balanceOf(this.InstaYieldStrategyAAVEv2Artifact.address)).to.be.eq(
    ...
    await expect(
      this.InstaYieldStrategyAAVEv2Artifact.connect(this.signers[2]).withdrawERC20(tokensToWithdraw, amountsToWithdraw),
    ...
    await aDaiContract
      .connect(this.signers[0])
      .transfer(this.InstaYieldStrategyAAVEv2Artifact.address, amountsToWithdraw2[0]);

    await expect(
      this.InstaYieldStrategyAAVEv2Artifact.connect(this.signers[0]).withdrawERC20(tokensToWithdraw2, amountsToWithdraw2),
  ...
  it("should revert with ARRAY SIZE MISMATCH when providing invalid input to withdrawERC20", async function () {
    ...
    await expect(
      this.InstaYieldStrategyAAVEv2Artifact.withdrawERC20(tokensToWithdraw, amountsToWithdraw),
    ).to.be.revertedWith("ARRAY SIZE MISMATCH");
  });

  it("deposit 10,000 aDAI and ensure yield balance increases by 10,000", async function () {
    const _amount = BigNumber.from(10000).mul(multiplier);
    const yieldBalanceBefore = await this.InstaYieldStrategyAAVEv2Artifact.getYieldBalance();
    ...
    await daiContract.connect(signer).transfer(this.InstaYieldStrategyAAVEv2Artifact.address, _amount);

    const yieldBalanceAfter = await this.InstaYieldStrategyAAVEv2Artifact.getYieldBalance();
  ...
  it("ensure quoting >totalSupply does not result in more yield than total yield available", async function () {
    ...
    // Get a quote
    const quotedResult = await this.InstaYieldStrategyAAVEv2Artifact.quoteBurnFToken(_amount);

    // Get the total available yield
    const totalYield = await this.InstaYieldStrategyAAVEv2Artifact.getYieldBalance();
