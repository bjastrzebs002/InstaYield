...
describe("InstaYield Tests (DAI)", function () {
  ...
  let protocolContract: InstaYieldProtocol;
  let strategyContract: InstaYieldStrategyAAVEv2;
  ...
  before(async function () {
    ...
    // 0. Deploy the InstaYieldNFT
    const nftArtifact: Artifact = await hre.artifacts.readArtifact("InstaYieldNFT");
    const nftContract = <InstaYieldNFT>await deployContract(signers[0], nftArtifact);

    // 0.1. Deploy the Flash FToken Factory contract
    const fTokenFactoryArtifact: Artifact = await hre.artifacts.readArtifact("InstaYieldFTokenFactory");
    const fTokenFactoryContract = <InstaYieldFTokenFactory>await deployContract(signers[0], fTokenFactoryArtifact);

    // 1. Deploy the Flash Protocol contract
    const protocolArtifact: Artifact = await hre.artifacts.readArtifact("InstaYieldProtocol");
    protocolContract = <InstaYieldProtocol>(
    ...
    const InstaYieldProtocolAddress = protocolContract.address;

    const strategyArtifact: Artifact = await hre.artifacts.readArtifact("InstaYieldStrategyAAVEv2");
    strategyContract = <InstaYieldStrategyAAVEv2>(
  ...
  it("account 0 should deploy Flash Token contract", async function () {
    ...
    InstaYieldTokenContract = <InstaYieldToken>await deployContract(signers[0], tokenArtifact);
  });
  ...
  it("should transfer NFT 1 from account 2 to account 3", async function () {
    const nftAddress = await protocolContract.InstaYieldNFTAddress();
    const InstaYieldNFTContract = await hre.ethers.getContractAt("InstaYieldNFT", nftAddress);

    expect(await InstaYieldNFTContract.ownerOf(1)).to.be.eq(signers[2].address);

    await InstaYieldNFTContract.connect(signers[2]).transferFrom(signers[2].address, signers[3].address, 1);

    expect(await InstaYieldNFTContract.ownerOf(1)).to.be.eq(signers[3].address);
  });
  ...
  it("should unstake with NFT as account 3 and receive initial principal", async function () {
    ...
    const nftAddress = await protocolContract.InstaYieldNFTAddress();
    const nftContract = await hre.ethers.getContractAt("InstaYieldNFT", nftAddress);
  ...
  it("(2) should issue nft for stake id 5 as account 5", async function () {
    ...
    const nftContract = await hre.ethers.getContractAt("InstaYieldNFT", await protocolContract.InstaYieldNFTAddress());
  ...
  it("account 6 should InstaYield 1000 DAI and wallet DAI balance is > 0", async function () {
    const _amount = BigNumber.from(1000).mul(multiplier);
    ...
    await protocolContract
      .connect(signers[6])
      .InstaYield(strategyContract.address, _amount, _duration, 0, signers[6].address, false);
    ...
    // be some DAI in the wallet again (the yield they generated during the InstaYield)
  });
  ...
  it("account 0 should approve spending of 150,000 Flash tokens against Rewards contract", async function () {
    ...
    await InstaYieldTokenContract.connect(signers[0]).approve(userIncentiveContract.address, _amount);
  });

  it("non-owner should fail when depositing reward with error Ownable: caller is not the owner", async function () {
    const _tokenAddress = InstaYieldTokenContract.address;
  ...
  it("account 7 should burn 10 fERC20 tokens and receive 5 FLASH tokens", async function () {
    ...
    expect(await InstaYieldTokenContract.balanceOf(signers[7].address)).to.be.eq(ethers.utils.parseUnits("5", 18));
  });
  ...
  it("account 7 should burn 10 fERC20 tokens and receive 7.5 FLASH tokens", async function () {
    ...
    expect(await InstaYieldTokenContract.balanceOf(signers[7].address)).to.be.eq(ethers.utils.parseUnits("12.5", 18));
  });
  ...
  it("account 0 should deposit 1,000 FLASH @ 1.5 ratio", async function () {
    ...
    const oldRewardBalance = await InstaYieldTokenContract.balanceOf(signers[0].address);
    ...
    const newRewardBalance = await InstaYieldTokenContract.balanceOf(signers[0].address);
  ...
  it("account 7 should burn 700 fERC20 tokens and receive 1,000 FLASH tokens at account 8", async function () {
    ...
    expect(await InstaYieldTokenContract.balanceOf(signers[8].address)).to.be.eq(ethers.utils.parseUnits("1000", 18));
  ...
  it("account 9 should InstaYield 1000 DAI and redirect yield to account 10", async function () {
    const _amount = BigNumber.from(1000).mul(multiplier);
    const _duration = 63072000;
    ...
    await protocolContract
      .connect(signers[9])
      .InstaYield(strategyContract.address, _amount, _duration, 0, signers[10].address, false);
